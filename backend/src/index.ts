...
const io = new SocketIOServer(server, {
  cors: {
    origin: ['https://cyberskillscmq.vercel.app', 'http://localhost:3000'],
    methods: ['GET', 'POST'],
    credentials: true,
  },
});

// Stockage de l'instance Socket.IO dans l'application
app.set('io', io);

// Middleware CORS pour les routes HTTP
app.use(cors({
  origin: ['https://cyberskillscmq.vercel.app', 'http://localhost:3000'],
  credentials: true,
}));
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (token: string) => {
      ...
      try {
        ...
        socket.emit('authError', { message: `Token invalide: ${err.message}` });
        socket.disconnect();
      }
    });
    ...
    socket.on('start-game', async (payload) => {
      try {
        ...
        console.log(`start-game reçu pour gameId: ${gameId}, socketId: ${socket.id}, userId: ${decoded.userId}`);
        ...
        console.log(`game-started émis pour gameId: ${gameId}`);
